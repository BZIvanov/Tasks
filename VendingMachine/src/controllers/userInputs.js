...
const vendingChange = (userBalance, item) => {
  ...
  for (let note of notes) {
    while (
      change >= banknoteAsNumberValue[note] &&
      currentChangeNotes[note] > 0
    ) {
...
const handleUserRequest = (userInput) => {
  if (products[userInput].quantity === 0) {
    messageTheUser(
      `Sorry, we are out of ${products[userInput].productName} products!`
    );
  } else if (products[userInput].price > userBalance.amount) {
    messageTheUser(
      `Sorry, you don't have enough money to buy ${products[userInput].productName}`
    );
  } else {
