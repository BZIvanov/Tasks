function validateFields(rawData: string, cacheStatus: string[]) {
  ...
  const report = {
    ...
    incorrectIndexes: cacheErrors.incorrectIndexes.concat(
      fieldsErrors.incorrectIndexes
    ),
  };
  return report;
}
...
function checkCacheStatus(rows: string[], cacheStatuses: string[]) {
  ...
  for (let s = 0; s < cacheStatuses.length; s++) {
    if (cacheStatuses[s] !== 'hit' && cacheStatuses[s] !== 'miss') {
      ...
      cacheErrors.errors.push(
        `Invalid cache status in ${errorText} on line ${s + 2}`
      );
...
function checkFieldsLength(rows: string[]) {
  ...
  for (let row = 0; row < rowsArray.length; row++) {
    ...
    while (startQuote > 0) {
      let originalStr: string = rowsArray[row].substring(
        startQuote + 1,
        endQuote
      );
      let replacedStr: string = rowsArray[row]
        .substring(startQuote + 1, endQuote)
        .replace(/ /g, '_');
    ...
    if (len !== 18) {
      lengthErrors.errors.push(
        `Expecting 18 fields but received ${len} on line ${row + 2}`
      );
